...
class AdamTraining:
    ...
    """
    def __init__(self, mu=2.0, rc = 1.5, eta=0.01, beta1=0.9, beta2=0.999, epsilon=1e-8):
    ...
    def getLamb(self, Lambdas):
        ...
        self.error = np.sum(np.absolute(np.triu(self.phi_sim, k=1) - np.triu(self.phi_exp, k=1)))/np.sum(np.triu(self.phi_exp, k=1))
